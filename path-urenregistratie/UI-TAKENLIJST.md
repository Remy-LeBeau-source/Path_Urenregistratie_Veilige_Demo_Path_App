...
| 33 | Testdocumenten bij Openen/Downloaden in het documentarchief. De factuur mag geen testplaceholder meer zijn maar een echte Path-factuur-PDF; bij de urenstaat mag een voorbeelddocument blijven, maar dan als echte Path-voorbeeldurenstaat i.p.v. vier regels tekst. Waar het zit: `test_reset_seed_documents()` in `server/lib/test-reset.php:448` schrijft voor béide soorten dezelfde 4-regelige `simple_pdf_text_document()`. Aanpak: (1) in `server/api/invoices.php` het opbouwen van de bytes losknippen van het opslaan — `invoices_generate_and_store_pdf()` (regel 309) bouwt nu `$lines` en roept meteen `invoices_store_pdf_bytes()`; splits dat in een `invoices_build_pdf_bytes()` die alleen bytes teruggeeft. (2) De reset schrijft die bytes op de bestaande `pdf_storage_key` — niet via `invoices_store_pdf_bytes()`, want die zet een nieuwe sleutel en dat laat de e2e-isolatievingerafdruk (`server/scripts/e2e-state-inspect.php:264`) per reset schuiven. (3) Voor de urenstaat de query in dezelfde functie uitbreiden met employees/periods/assignments, zodat naam, klant, maand en uren in een echte voorbeeldstaat kunnen. Let op: de marker `TESTDOCUMENT` moet in de urenstaat blijven staan, want `server/mail/dispatch.php:170` gebruikt die om te voorkomen dat een placeholder als factuur wordt meegemaild (vastgelegd in de smoke test, regel 2039). Main-terrein (server + factuurlogica). | foto gebruiker | open, main |
...
| 35 | Voettekst van het beheerdashboard toont in New nog de Klassiek-stijl (screenshot met "© 2026 Path Consultancy IT"). Niet apart opgemaakt in deze skin. | eigen vondst in foto gebruiker | open |
| 36 | Releasepijplijn op main is rood, vier runs op rij, dus er komt niets meer op TEST. Drie oorzaken: (a) SAFE-H-018 faalt zelf ("A remote TEST demo account does not match the canonical login baseline"), (b) test-design-audit klaagt "Executable cases zonder feature: SAFE-H-018", (c) shard 9 loopt op 124 (de releasepijplijn staat nog op 10 shards en 15 min terwijl CI naar 22 min is gegaan). Alle drie main-terrein. | eigen vondst | open, main |
