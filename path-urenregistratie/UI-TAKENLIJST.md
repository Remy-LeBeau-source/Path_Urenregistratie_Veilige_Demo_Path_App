...
| 34 | Beheerdashboard in New: alleen de kolom breder gemaakt (1560px naar 1840px), de "veel lege ruimte" is weg. Het tweede deel van mijn voorstel -- Open acties per maand en Klanturenstaten elk naar een eigen scherm -- heb ik bewust niet gedaan: 4 testbestanden ankeren op `#admin-task-panel`/`#customer-timesheet-admin-panel` terwijl die op `#view-dashboard` staan, waaronder het grootste, meest kwetsbare specbestand (dashboard.spec.ts). Dat raakt de harde eis dat functionaliteit moet blijven werken en verdient een eigen sessie met volledige regressie ervoor en erna, niet iets wat ik erbij doe. | vraag gebruiker | deels klaar (v1.0.100), rest open |
