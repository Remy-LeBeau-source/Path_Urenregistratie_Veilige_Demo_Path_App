...
assert(dom.window.localAccountToolsAllowed("localhost") && dom.window.localAccountToolsAllowed("127.0.0.1") && !dom.window.localAccountToolsAllowed("uren-test.pathconsultancy.nl"), "Lokale reset- en hintfuncties mogen alleen op loopbackhosts beschikbaar zijn");
...
assert(document.querySelector(".demo-badge").textContent.includes("0.9.61"), "Het zichtbare versienummer moet 0.9.61 zijn");
...
assert(demoScenarioState.schemaVersion === 26, "Versie 0.9.61 moet wijzigingen onder de juiste gegevensversie bewaren");
...
assert(migratedState.schemaVersion === 26, "Bestaande browsergegevens moeten ook in v0.9.61 veilig behouden blijven");
...
dom.window.close();
console.log("Path v0.9.61 volledige smoke test: geslaagd");
