...
assert(document.querySelector(".demo-badge").textContent.includes("0.9.15"), "Het zichtbare versienummer moet 0.9.15 zijn");
...
assert(demoScenarioState.schemaVersion === 23, "Versie 0.9.15 moet wijzigingen onder de juiste gegevensversie bewaren");
...
assert(!document.querySelector("#admin-task-panel").hidden && document.querySelector("#admin-task-panel").textContent.includes("Alle open taken") && document.querySelector("#admin-task-panel").textContent.includes("Zonder maanden wisselen"), "De beheerder moet één taakgerichte werkvoorraad krijgen die niet door de gekozen maand wordt beperkt");
...
assert(document.querySelectorAll("#admin-task-list [data-admin-task-row]").length === dashboardTasks.length, "Alle open taken moet standaard iedere taak exact één keer tonen");
...
assert(document.querySelector("#admin-task-summary").textContent.includes(dashboardTasks.length + " open taken over " + dashboardTaskMonths + " maanden") && document.querySelector("#admin-task-summary").textContent.includes(dashboardActionableTasks.length + " bij Backoffice") && document.querySelector("#admin-task-summary").textContent.includes(dashboardWaitingTasks.length + " bij medewerkers") && document.querySelector("#admin-task-summary").textContent.includes("Elke regel hieronder telt als 1 taak"), "De werkvoorraad moet het totaal één-op-één uitsplitsen naar eigenaar");
...
assert(document.querySelector("#admin-attention-note").textContent === "3 open taken over 2 maanden · 0 bij Backoffice · 3 bij medewerkers.", "Het hoofdtotaal moet exact gelijk zijn aan de verdeling per eigenaar");
assert(document.querySelector("#metric-actions").textContent === "3" && document.querySelector("#metric-actions-note").textContent === "0 bij Backoffice · 3 bij medewerkers · 2 maanden", "De taakkaart moet dezelfde één-op-één telling tonen");
assert(document.querySelector("#open-work-queue").textContent === "Bekijk alle open taken · 3", "De hoofdknop moet altijd naar het volledige taaktotaal verwijzen");
assert(document.querySelector("#open-delivery-check").textContent === "Augustus 2026 · 2 urentaken bij medewerkers", "De maandknop moet expliciet zeggen dat twee van de drie taken in augustus bij medewerkers liggen");
...
assert(document.querySelector("#open-periods-panel").textContent.includes("Samenvatting per maand") && document.querySelector("#open-periods-panel").textContent.includes("Alleen voor overzicht"), "Het maandoverzicht moet ondergeschikt zijn aan de taakgerichte werkvoorraad");
...
const dashboardPeriodSummaries = dom.window.openPeriodSummaries();
const julyDashboardSummary = dashboardPeriodSummaries.find(item => item.periodKey === "2026-07");
const augustDashboardSummary = dashboardPeriodSummaries.find(item => item.periodKey === "2026-08");
assert(julyDashboardSummary && document.querySelector('[data-open-period-row="2026-07"]').textContent.includes(julyDashboardSummary.openCount + " openstaande") && document.querySelector('[data-open-period="2026-07"][data-open-period-section="customer-timesheets"]'), "De julirij moet het actuele taaktotaal en de klanturenstaatactie tonen");
assert(augustDashboardSummary && document.querySelector('[data-open-period-row="2026-08"]').textContent.includes(augustDashboardSummary.openCount + " openstaande"), "De augustusrij moet het actuele taaktotaal tonen");
assert(document.querySelector("#metric-actions").closest(".metric-card").textContent.includes("Open taken") && Number(document.querySelector("#metric-actions").textContent) === dashboardTasks.length && document.querySelector("#metric-actions-note").textContent.includes(dashboardActionableTasks.length + " bij Backoffice") && document.querySelector("#metric-actions-note").textContent.includes(dashboardWaitingTasks.length + " bij medewerkers") && document.querySelector("#metric-actions-note").textContent.includes(dashboardTaskMonths + " maanden"), "Open taken moet het actuele totaal en de eigenaarsverdeling uitsplitsen");
choosePeriod("#period-month-picker", "#period-year-picker", "2026-08");
const augustWorkflowTasks = dom.window.adminOpenTasks().filter(task => task.periodKey === "2026-08");
assert(document.querySelector("#workflow-open-count").textContent.includes(augustWorkflowTasks.length + " open taken") && document.querySelector("#workflow-open-breakdown").textContent.includes("klanturensta"), "De procesbalk moet het actuele maandtotaal en de aparte klanturenstaten benoemen");
...
assert(document.querySelector("#dashboard-team-title").textContent === "Uren wachten op medewerker · Augustus 2026 · 2 medewerkers", "De gefilterde teamstatus moet expliciet zeggen dat alleen twee medewerkers met open uren in augustus worden getoond");
...
assert(document.querySelector("#period-label").textContent === "Juli 2026" && document.querySelector("#view-dashboard").classList.contains("is-active"), "Een actie vanuit de maandsamenvatting moet precies die maand en het juiste onderdeel openen");
choosePeriod("#period-month-picker", "#period-year-picker", "2026-08");
...
assert(document.querySelector("#open-delivery-check").textContent.includes("Juli 2026") && document.querySelector("#open-delivery-check").textContent.includes("urencontrole") && document.querySelector("#open-delivery-check").textContent.includes("bij Backoffice"), "De dashboardactie moet de geselecteerde maand, taaksoort en eigenaar expliciet noemen");
click("#open-delivery-check");
assert(document.querySelector("#view-approvals").classList.contains("is-active"), "De maandactie moet rechtstreeks naar de genoemde urencontrole bij Backoffice gaan");
click('[data-view="approvals"]');
...
assert(Number(document.querySelector("#metric-actions").textContent) > 0 && document.querySelector("#metric-actions-note").textContent.includes("maand"), "Het actietotaal moet onafhankelijk van de gekozen detailmaand alle open maanden blijven tonen");
click('[data-open-period="2026-07"][data-open-period-section="customer-timesheets"]');
...
assert(migratedState.schemaVersion === 23, "Bestaande browsergegevens moeten ook in v0.9.15 veilig behouden blijven");
...
console.log("Path v0.9.15 volledige smoke test: geslaagd");
