...
assert(document.querySelector(".demo-badge").textContent.includes("v0.9.2"), "De zichtbare demoversie moet v0.9.2 zijn");
...
assert(document.querySelector("#metric-approved-action").textContent.includes("Open 1 controle"), "Vanuit de goedkeuringskaart moet een concrete doorklikactie voor de gekozen maand staan");
assert(document.querySelector("#hours-total").textContent === "153,0", "Juli moet voor Stasjo van Bakel 153,0 uur tonen");
assert(document.querySelectorAll("#approval-list .approval-card").length === 2, "De gevulde demo moet de open goedkeuringen uit juli en augustus tonen");
...
assert(demoScenarioState.schemaVersion === 20, "Versie 0.9.2 moet wijzigingen onder de juiste gegevensversie bewaren");
assert(demoScenarioState.records["2026-07"]["1"].invoiceStatus === "ready" && demoScenarioState.records["2026-07"]["2"].invoiceStatus === "ready" && demoScenarioState.records["2026-07"]["4"].invoiceStatus === "ready", "Juli moet direct drie bevestigde conceptfacturen voor de demo klaarzetten");
...
assert(document.querySelector("#open-delivery-check").hidden, "Het dashboard mag geen maandcontrole tonen als in de gekozen maand niets klaarstaat");
assert(!document.querySelector("#open-periods-panel").hidden, "Het dashboard moet oudere open maanden blijven tonen, ook wanneer een lege andere maand is geselecteerd");
const openPeriodKeys = [...document.querySelectorAll("[data-open-period-row]")].map(item => item.dataset.openPeriodRow);
assert(openPeriodKeys.includes("2026-07") && openPeriodKeys.includes("2026-08") && !openPeriodKeys.includes("2026-06"), "De gevulde demo moet juli en augustus open tonen en afgerond juni weglaten");
assert(JSON.stringify(openPeriodKeys) === JSON.stringify([...openPeriodKeys].sort()), "Openstaande maanden moeten altijd met de oudste maand bovenaan staan");
assert(document.querySelector('[data-open-period-row="2026-07"]').textContent.includes("1 goedkeuring") && document.querySelector('[data-open-period-row="2026-07"]').textContent.includes("3 verzendingen"), "Juli moet de open goedkeuring en drie verzendcontroles samenvatten");
click('[data-open-period="2026-07"]');
assert(document.querySelector("#period-label").textContent === "Juli 2026", "Een actie vanuit Openstaande maanden moet eerst precies die maand selecteren");
assert(document.querySelector("#view-approvals").classList.contains("is-active"), "Een maand met een open urencontrole moet rechtstreeks de juiste goedkeuringslijst openen");
...
assert(document.querySelectorAll("#approval-list .approval-card").length === 2, "Openstaande uren uit juli en augustus moeten samen zichtbaar zijn");
...
click('[data-approval-scope="all"]');
assert(document.querySelectorAll("#approval-list .approval-card").length === 2, "Alle openstaande moet de regels uit alle maanden weer tonen");
click('[data-view="invoices"]');
...
document.querySelector("#edit-new-recipient-category").value = "payroll";
...
document.querySelector("#edit-mail-recipient-category").value = "payroll";
...
assert(!document.querySelector("#open-delivery-check").hidden, "Het dashboard moet de gezamenlijke maandcontrole tonen wanneer verzendingen klaarstaan");
assert(document.querySelector("#open-delivery-check").textContent.includes("Maandverzending controleren · 3"), "De dashboardactie moet het aantal klaargezette medewerkerverzendingen tonen");
click("#open-delivery-check");
assert(document.querySelector("#view-invoices").classList.contains("is-active"), "De dashboardactie moet rechtstreeks naar de gezamenlijke verzendcontrole gaan");
click('[data-view="approvals"]');
...
click('[data-view="approvals"]');
click("[data-approve]");
assert(document.querySelectorAll("#approval-list .approval-card").length === 1, "Goedkeuren moet de openstaande lijst bijwerken");
...
click("#modal-confirm");
assert(document.querySelector("#invoice-rows").textContent.includes("Verzendtest afgerond"), "De verzendteststatus moet duidelijk worden vastgelegd");
...
assert(document.querySelectorAll("#invoice-rows tr").length >= 1, "Het filter Verzendtest afgerond moet facturen met een afgeronde verzendtest tonen");
click('[data-invoice-filter="all"]');
...
assert(document.querySelector("#view-invoices").classList.contains("is-active"), "De beheerder moet na een mailtest op Facturen blijven");
picker.value = "2026-08";
picker.dispatchEvent(new Event("change", { bubbles: true }));
...
assert(document.querySelector("#modal-confirm").textContent === "Verzendtest uitvoeren", "De demo mag niet suggereren dat de maand al echt wordt verzonden");
...
assert(migratedState.schemaVersion === 20, "Bestaande browsergegevens moeten automatisch naar v0.9.2 migreren");
...
console.log("Path demo v0.9.2 volledige smoke test: geslaagd");
