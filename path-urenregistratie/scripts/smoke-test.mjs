...
assert(document.querySelector(".demo-badge").textContent.includes("0.9.12"), "Het zichtbare versienummer moet 0.9.12 zijn");
...
assert(demoScenarioState.schemaVersion === 22, "Versie 0.9.12 moet wijzigingen onder de juiste gegevensversie bewaren");
...
assert(demoScenarioState.records["2026-07"]["2"].customerTimesheet.status === "received" && demoScenarioState.records["2026-07"]["3"].customerTimesheet.status === "missing", "De gevulde demo moet ontvangen en ontbrekende klanturenstaten bevatten");
...
assert(dom.window.openPeriodSummaries().find(item => item.periodKey === "2026-07").counts.customerDocuments === 3, "Een als rechtstreeks gemaild geregistreerde klanturenstaat moet uit de open werkvoorraad verdwijnen");
...
assert(document.querySelector("#open-periods-panel").textContent.includes("Werkvoorraad over alle maanden") && document.querySelector("#open-periods-panel").textContent.includes("zelf maanden nalopen is niet nodig"), "Het dashboard moet één centrale werkvoorraad over alle maanden uitleggen");
...
assert(document.querySelector('[data-open-period-row="2026-07"]').textContent.includes("4 klanturenstaten open") && document.querySelector('[data-open-period="2026-07"][data-open-period-section="customer-timesheets"]'), "Ontbrekende, te controleren en nog te verzenden klanturenstaten moeten in de maandoverschrijdende werkvoorraad staan");
assert(Number(document.querySelector("#metric-actions").textContent) > 2 && document.querySelector("#metric-actions-note").textContent.includes("oudste eerst"), "Open acties moet het echte aantal taken over alle maanden tonen en niet alleen het aantal actietypen van de gekozen maand");
choosePeriod("#period-month-picker", "#period-year-picker", "2026-08");
assert(document.querySelector('[data-open-period="2026-08"][data-open-period-section="team"]'), "Een maand met concepten of correcties moet in de centrale werkvoorraad een concrete teamstatusactie tonen");
click('[data-open-period="2026-08"][data-open-period-section="team"]');
assert(document.querySelector("#view-dashboard").classList.contains("is-active"), "Teamstatus moet op het dashboard blijven in plaats van stamgegevens te openen");
assert(document.querySelector("#dashboard-team-title").textContent === "Teamstatus · Augustus 2026 · 2 nog niet klaar", "De gefilterde teamstatus moet periode en aantal duidelijk tonen");
assert(document.querySelectorAll("#dashboard-employee-rows tr").length === 2, "Alleen medewerkers met concepturen of een correctie moeten in de teamstatus staan");
assert(document.querySelector("#dashboard-employee-rows").textContent.includes("Marc de Roon") && document.querySelector("#dashboard-employee-rows").textContent.includes("Stasjo van Bakel"), "De twee medewerkers die actie nodig hebben moeten zichtbaar zijn");
assert(!document.querySelector("#dashboard-employee-rows").textContent.includes("Brian Hek") && !document.querySelector("#dashboard-employee-rows").textContent.includes("Shawn-Douglas Nahar"), "Afgeronde of reeds ingediende medewerkers horen niet in het aandachtsfilter");
assert(document.querySelector("#dashboard-employee-rows").textContent.includes("Details bekijken") && !document.querySelector("#dashboard-employee-rows [data-dashboard-timesheet]"), "De teamstatus moet voor beheerders naar een alleen-lezen urendetail gaan");
...
click("#modal-confirm");
assert(document.querySelector("#customer-timesheet-admin-list").textContent.includes("Nog niet ontvangen") && document.querySelector("#customer-timesheet-admin-list").textContent.includes("Opnieuw uploaden"), "Backoffice moet ontbrekende en opnieuw te uploaden klanturenstaten direct zien");
...
click('[data-remind-customer-timesheet="1"]');
const customerReminderState = JSON.parse(dom.window.localStorage.getItem("path-uren-demo-v07-final"));
assert(customerReminderState.notifications.length === notificationsBeforeCustomerReminder + 1 && customerReminderState.notifications.at(-1).title === "Klanturenstaat ontbreekt", "Backoffice moet handmatig een klanturenstaatherinnering kunnen klaarzetten");
click('#dashboard-team-clear-filter');
assert(document.querySelectorAll("#dashboard-employee-rows tr").length === 4, "Toon volledig team moet alle actieve medewerkers terugbrengen");
...
click('[data-review-customer-timesheet="2"]');
assert(document.querySelector("#modal-title").textContent.includes("Stasjo van Bakel") && document.querySelector("#modal-secondary").textContent === "Opnieuw uploaden vragen", "Backoffice moet een officiële PDF kunnen goedkeuren of opnieuw laten uploaden");
assert(document.querySelector('#modal-summary [data-view-customer-timesheet="2"]'), "Backoffice moet de ingediende PDF kunnen bekijken voordat deze wordt goedgekeurd");
assert(document.querySelector("#modal-summary").textContent.includes("Van Stasjo van Bakel aan Path Backoffice") && document.querySelector("#modal-summary").textContent.includes("mijn door de klant verstrekte klanturenstaat"), "De beheerder moet eerst het bericht van de medewerker bij de inzending zien");
click("#modal-confirm");
assert(JSON.parse(dom.window.localStorage.getItem("path-uren-demo-v07-final")).records["2026-07"]["2"].customerTimesheet.status === "approved", "Een gecontroleerde klanturenstaat moet Goedgekeurd worden");
click('[data-send-customer-timesheet="2"]');
assert(document.querySelector("#modal-summary").textContent.includes("urenstaten-itaq@example.invalid") && document.querySelector("#modal-summary").textContent.includes("Voorbeeld_Klanturenstaat_Stasjo_van_Bakel_2026-07.pdf"), "De aparte brokercontrole moet het afwijkende adres en de herkenbare voorbeeld-PDF tonen");
...
assert(document.querySelector("#customer-timesheet-broker-subject").value === "Klanturenstaat Stasjo van Bakel – juli 2026" && document.querySelector("#customer-timesheet-broker-body").value.includes("Path Backoffice") && document.querySelector("#customer-timesheet-broker-body").value.includes("Path Consultancy"), "Pas na goedkeuring moet Backoffice een apart brokerbericht met de juiste ondertekening krijgen");
document.querySelector("#customer-timesheet-broker-subject").value = "Aangepast onderwerp juli 2026";
document.querySelector("#customer-timesheet-broker-body").value = "Aangepaste begeleidende tekst.";
click("#modal-confirm");
assert(JSON.parse(dom.window.localStorage.getItem("path-uren-demo-v07-final")).records["2026-07"]["2"].customerTimesheet.status === "sent", "Na controle moet de aparte klanturenstaatroute als gecontroleerd worden vastgelegd");
assert(JSON.parse(dom.window.localStorage.getItem("path-uren-demo-v07-final")).records["2026-07"]["2"].customerTimesheet.brokerSubject === "Aangepast onderwerp juli 2026", "Een aangepaste klanturenstaatmail moet bij de brokercontrole bewaard blijven");
click('[data-admin-hours-detail="1"]');
...
assert(document.querySelector("#test-month-delivery").textContent.includes("1 openstaand"), "Bij een open urenstaat moet de maandknop direct het openstaande aantal tonen");
assert(document.querySelector("#invoice-rows").textContent.includes("Urencontrole nodig") && document.querySelector("#invoice-rows").textContent.includes("Uren goedkeuren"), "De factuurlijst moet een open urencontrole met een directe actie tonen");
click("#help-launcher");
...
assert(document.querySelector("#approval-period-title").textContent === "Alle openstaande uren · 2", "Goedkeuringen moeten vanuit het hoofdmenu standaard alle maanden tonen");
...
assert(document.querySelector('[data-approval-scope="all"]').textContent.includes("2"), "Het totaalfilter moet direct het aantal openstaande urenstaten tonen");
assert(document.querySelector("#global-period-control").hidden, "De algemene maandkiezer moet bij Goedkeuringen verborgen zijn");
assert(document.querySelector("#approval-period-filters").textContent.includes("Juli 2026 · 1") && document.querySelector("#approval-period-filters").textContent.includes("Augustus 2026 · 1"), "Goedkeuringen moeten alleen maanden tonen waarin werkelijk iets openstaat");
...
assert(document.querySelector("#approve-all").textContent === "Alle 2 goedkeuren", "De bulkknop moet het concrete aantal open urenstaten noemen");
click('[data-approval-filter-period="2026-08"]');
assert(document.querySelector("#approval-period-title").textContent === "Openstaande uren · Augustus 2026", "De maandtab moet bewust op de gekozen maand filteren");
...
assert(document.querySelector("#approve-all").textContent === "1 urenstaat in augustus goedkeuren", "De maandknop moet aantal en maand concreet en grammaticaal juist noemen");
...
document.querySelector("#edit-customer-timesheet-due-day").value = "7";
...
click("#save-settings");
document.querySelector("#setting-weekly-reminder-day").value = "thursday";
document.querySelector("#setting-weekly-reminder-time").value = "12:00";
document.querySelector("#setting-weekly-reminder-time").dispatchEvent(new Event("change", { bubbles: true }));
assert(document.querySelector("#reminder-schedule-summary").textContent.includes("donderdag 12:00"), "De herinneringssamenvatting moet wijzigingen direct tonen");
assert(document.querySelector("#reminder-schedule-summary").textContent.includes("klanturenstaat") && document.querySelector("#reminder-schedule-summary").textContent.includes("na 2 werkdagen"), "De herinneringssamenvatting moet de aparte klanturenstaatplanning uitleggen");
click("#save-settings");
...
assert(document.querySelector("#open-delivery-check").textContent.includes("1 openstaand"), "De dashboardactie moet eerst waarschuwen als de gekozen maand nog niet volledig klaar is");
...
click('[data-view="dashboard"]');
assert(document.querySelector("#admin-attention-note").textContent.includes("verdeeld over") && !document.querySelector("#open-work-queue").hidden, "Een lege geselecteerde maand mag open acties uit eerdere maanden niet verbergen");
assert(Number(document.querySelector("#metric-actions").textContent) > 0 && document.querySelector("#metric-actions-note").textContent.includes("oudste eerst"), "Het actietotaal moet onafhankelijk van de gekozen detailmaand alle open maanden blijven tonen");
click('[data-open-period="2026-07"][data-open-period-section="customer-timesheets"]');
assert(document.querySelector("#period-label").textContent === "Juli 2026" && document.querySelector("#customer-timesheet-admin-title").textContent.includes("Juli 2026") && document.querySelector("#view-dashboard").classList.contains("is-active"), "Vanuit de werkvoorraad moet één klik de juiste maand en klanturenstaten openen");
assert(document.querySelector("#close-progress-title").textContent === "0 van 4 stappen voltooid" && document.querySelector("#close-progress-note").textContent.includes("Uren → Controle → Facturen → Verzending"), "De vier stappen moeten direct in het voortgangsblok worden uitgelegd");
...
assert(migratedState.schemaVersion === 22, "Bestaande browsergegevens moeten automatisch naar v0.9.12 migreren");
...
console.log("Path v0.9.12 volledige smoke test: geslaagd");
