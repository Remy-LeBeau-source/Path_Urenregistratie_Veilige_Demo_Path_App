...
//
// Dit staat hier omdat twee sessies er op 13 sep 2026 onafhankelijk van elkaar
// in zijn getrapt: allebei concludeerden we dat het script "hing", allebei op
// grond van een te korte timeout (180 s resp. 560 s) en nul output. Hij hing
// niet. Hij verbruikt in zijn eentje ruim 900 CPU-seconden: de volledige
// assets/app.js (ruim 800 KB) wordt in JSDOM geëvalueerd en daarna volgen
// honderden renderAll()-rondes.
//
// Hoe je "traag" van "vastgelopen" onderscheidt, zonder te gokken: bemonster de
// CPU-tijd twee keer. Op Windows:
//   Get-CimInstance Win32_Process -Filter "Name='node.exe'" |
//     Select ProcessId, @{n='CPUsec';e={($_.UserModeTime+$_.KernelModeTime)/1e7}}
// Loopt die waarde door tussen twee metingen, dan rekent hij en hangt hij niet.
//
// Tweede valkuil uit diezelfde dag: het afbreken van een achtergrondtaak laat
// het node-kindproces vaak gewoon doordraaien. Drie exemplaren tegelijk maakten
// één run drie keer zo traag. Controleer de processenlijst voordat je iets over
// de snelheid concludeert.
...
// JSDOM biedt requestAnimationFrame alleen met pretendToBeVisual. De statische
// smoke controleert geen animatieframes; de echte browsercases doen dat wel.
// Een no-op houdt deze DOM-contracttest vrij van extra timinggedrag.
...
//
// 17 sep 2026: standaard van 4000 naar 30000 ms. Op deze machine duurt een
// klanturenstaat opslaan gemeten 23,3 s, indienen 14,0 s en een PNG-concept
// 16,2 s -- ruim boven de oude 4 s. In de pipeline zijn dezelfde stappen ruim
// binnen de grens; dit is puur het geduld van de lokale test aanpassen aan
// wat hier echt gemeten is, geen verzwakking van de asserties zelf.
...
assert(document.querySelector(".demo-badge").textContent.includes("2.0.158"), "Het zichtbare versienummer moet 2.0.158 zijn");
...
assert(demoScenarioState.schemaVersion === 27, "Versie 0.9.81 moet wijzigingen onder de juiste gegevensversie bewaren");
...
assert(document.querySelector("#employee-customer-timesheet-status").textContent === "Wacht op Backoffice" && document.querySelector("#employee-customer-timesheet-skip").textContent === "Toch een bestand toevoegen", "De medewerker moet de eigen rechtstreekse verzending kunnen zien en terugdraaien");
const juneAfterCustomerSkipTasks = dom.window.adminOpenTasks().filter(task => task.periodKey === "2026-06");
assert(juneAfterCustomerSkipTasks.length === 3 && juneAfterCustomerSkipTasks.some(task => task.type === "customer-external-confirm" && task.employee.id === 3 && task.actionable), "Na Brians melding blijft de klanturenstaat als concrete externe bevestigingsactie voor Backoffice zichtbaar");
...
click("#modal-confirm");
click("#employee-customer-timesheet-skip");
assert(JSON.parse(dom.window.localStorage.getItem("path-uren-demo-v07-final")).records["2026-06"]["3"].customerTimesheet.status === "missing", "Toch een bestand toevoegen moet de registratie veilig terugzetten naar Nog niet ontvangen");
dom.window.setPeriod("2026-07");
...
assert(document.querySelector("#hours-target-help").textContent.includes("Automatisch opgeslagen."), "De controle moet duidelijk zeggen dat het verschil niet blokkeert");
...
// De maand van de klanturenstaat; het berichtvoorbeeld dat dit eerder liet zien is weg (besluit Gio 14 sep).
assert(document.querySelector("#customer-timesheet-period").value === "2026-08", "Een gewone maandknop moet de klanturenstaat direct wijzigen");
...
assert(document.querySelector("#customer-timesheet-period").value === "2027-01", "De klanturenstaatroute moet automatisch overgaan naar een volgend jaar");
...
assert(/werkdag/.test(document.querySelector("#hours-target-help").textContent), "Indienen moet expliciet tot één geselecteerde maand beperkt zijn");
...
assert(document.querySelector("#workspace-avatar").textContent === "JS", "Joyce van der Steenhoven moet initialen JS tonen: voornaam + achternaam, tussenvoegsels tellen niet mee");
...
// Sinds Klanturenstaten een eigen scherm heeft (v2.0.158) bestaat dezelfde
...
assert(document.querySelector('[data-announcement-archive-filter="actueel"]').classList.contains("is-active"), "Berichten moet starten op Actueel");
assert(!document.querySelector("#employee-announcement-list").textContent.includes("is ingetrokken"), "Onder Actueel horen ingetrokken berichten niet te staan");
...
assert(!document.querySelector("#employee-announcement-list [data-read-announcement]"), "De oude markeerknop (data-read-announcement) bestaat niet meer; het nieuwe knopje heet data-bericht-gelezen");
...
assert(migratedState.announcementArchiveFilter === "actueel", "Migratie moet het medewerkersarchief met het standaardfilter Actueel toevoegen (Gio 15 sep)");
...
dom.window.close();
console.log("Path v2.0.158 volledige smoke test: geslaagd");
