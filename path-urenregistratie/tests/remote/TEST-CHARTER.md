...
- **TEST-E2E-27** — heropen-beslissingstabel: een goedgekeurde urenstaat mag terug naar correctie zolang er geen definitief gemaakte factuur ligt — ook als er al een conceptfactuur klaarstaat, want die is nooit verstuurd en wordt bij definitief maken hoe dan ook opnieuw uit de urenstaat berekend. Zodra de factuur definitief is (`locked_at` gezet, status `invoiced`) wordt heropenen met 409 geweigerd en blijft de status ongewijzigd.
