...
| W10 | **Versieschema.** Op 3 sep 2026 naar `1.0.0` gesprongen; op 6 sep 2026 herzien: **terug naar `0.0.1`** en van daaruit `0.0.x` ophogen per commit. `1.x` claimde ten onrechte productie-rijp; de app draait nog niet echt live in de nieuwe vorm. De **echte productieversie** (`1.0.0`) komt pas bij go-live; daarna `1.0.x` fixes, `1.1.0` functies. Het nummer staat op 13 plekken in 5 bestanden; zet het nooit met de hand maar met **`npm run version:set <versie>`**. `npm run version:check` (draait mee in `npm run check`) faalt zodra ergens een ander nummer staat dan in `package.json`. | "1.x" leest als af terwijl de herontwerp-fase nog loopt; handmatig gelijkzetten was foutgevoelig. | vast |
